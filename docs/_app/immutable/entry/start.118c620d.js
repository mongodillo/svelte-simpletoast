import { o as Ce, t as ye } from '../chunks/index.6bb085b2.js';
import {
	S as Ge,
	a as Je,
	I as q,
	...
	P as Fe,
	h as We
} from '../chunks/singletons.bde5d5ef.js';
function Xe(t, o) {
	return t === '/' || o === 'ignore'
		? t
		: o === 'never'
...
window.fetch = (t, o) => (
	(t instanceof Request ? t.method : (o == null ? void 0 : o.method) || 'GET') !== 'GET' &&
		te.delete(Se(t)),
	fe(t, o)
);
const te = new Map();
	...
	if (te.size > 0) {
		...
		if (d) {
			if (
				...
				['default', 'force-cache', 'only-if-cached', void 0].includes(u == null ? void 0 : u.cache)
			)
				...
				: new RegExp(
						`^${ut(t)
							.map((i) => {
								...
								if (f)
									return (
										o.push({ name: f[1], matcher: f[2], optional: !0, rest: !1, chained: !0 }),
										'(?:/([^/]+))?'
									);
								if (!i) return;
								const S = i.split(/\[(.+?)\](?!\])/);
								return (
									...
									S.map((b, w) => {
										if (w % 2) {
											if (b.startsWith('x+'))
												return be(String.fromCharCode(parseInt(b.slice(2), 16)));
											if (b.startsWith('u+'))
												return be(
													String.fromCharCode(
														...b
...
function pt({ nodes: t, server_loads: o, dictionary: u, matchers: i }) {
	...
	return Object.entries(u).map(([l, [b, w, p]]) => {
		const { pattern: C, params: T } = lt(l),
			k = {
				id: l,
				exec: (N) => {
					const P = C.exec(N);
					if (P) return dt(P, T, i);
				},
			...
			if (typeof l[0] == 'string') {
				...
				switch (b) {
					...
					case 'null':
						...
						i[f] = T;
						for (let k = 1; k < l.length; k += 2) T[l[k]] = d(l[k + 1]);
						break;
					default:
...
function Rt(t, o) {
	...
	let p = { branch: [], error: null, url: null },
		...
		H = !1,
		F,
		$ = (xe = history.state) == null ? void 0 : xe[q];
	...
	function Oe(e) {
		...
		n && n.remove(),
			(M = e.props.page),
			(F = new t.root({ target: o, props: { ...e.props, stores: V, components: l }, hydrate: !0 })),
			Ae($);
		const r = {
			from: null,
			to: {
		...
		w.after_navigate.forEach((s) => s(r)), (T = !0);
	}
	async function Y({ url: e, params: n, branch: r, status: a, error: s, route: c, form: g }) {
		let m = 'never';
		...
		for (let _ = 0; _ < Math.max(r.length, p.branch.length); _ += 1) {
			const h = r[_],
				U = p.branch[_];
			(h == null ? void 0 : h.data) !== (U == null ? void 0 : U.data) && (R = !0),
				h && ((y = { ...y, ...h.data }), R && (v.props[`data_${L}`] = y), (L += 1));
		}
		return (
			(!p.url || e.href !== p.url.href || p.error !== s || (g !== void 0 && g !== M.form) || R) &&
				(v.props.page = {
					error: s,
					params: n,
	...
	async function de({ loader: e, parent: n, url: r, params: a, route: s, server_data_node: c }) {
		var y, R, L;
		let g = null;
		const m = { dependencies: new Set(), params: new Set(), parent: !1, route: !1, url: !1 },
			v = await e();
		if ((y = v.universal) != null && y.load) {
			let O = function (...h) {
				for (const U of h) {
					const { href: x } = new URL(U, r);
					m.dependencies.add(x);
				}
			};
			const _ = {
				route: {
				...
				async fetch(h, U) {
					...
					return (
						O(D.href),
						D.origin === r.origin && (x = D.href.slice(r.origin.length)),
						T ? st(x, D.href, U) : it(x, U)
					);
				},
				setHeaders: () => {},
				depends: O,
		...
		return {
			...
			loader: e,
			server: c,
			universal: (R = v.universal) != null && R.load ? { type: 'data', data: g, uses: m } : null,
			data: g ?? (c == null ? void 0 : c.data) ?? null,
			slash:
				((L = v.universal) == null ? void 0 : L.trailingSlash) ?? (c == null ? void 0 : c.slash)
		};
	}
	...
	async function he({ id: e, invalidating: n, url: r, params: a, route: s }) {
		...
		const _ = v.map((E, I) => {
			...
			const A = p.branch[I],
				j =
					!!(E != null && E[0]) &&
					((A == null ? void 0 : A.loader) !== E[1] ||
						Pe(O, L, R, (G = A.server) == null ? void 0 : G.uses, a));
			return j && (O = !0), j;
		});
		if (_.some(Boolean)) {
			try {
		...
		const x = v.map(async (E, I) => {
			...
			if (
				(!j || j.type === 'skip') &&
				E[1] === (A == null ? void 0 : A.loader) &&
				!Pe(U, L, R, (ge = A.universal) == null ? void 0 : ge.uses, a)
			)
				return A;
			if (((U = !0), (j == null ? void 0 : j.type) === 'error')) throw j;
			return de({
				loader: E[1],
				url: r,
				params: a,
				...
				parent: async () => {
					...
					for (let me = 0; me < I; me += 1)
						Object.assign($e, (je = await x[me]) == null ? void 0 : je.data);
					return $e;
				},
				server_data_node: pe(
					j === void 0 && E[0] ? { type: 'skip' } : j ?? null,
					E[0] ? (A == null ? void 0 : A.server) : void 0
				)
			});
		});
		for (const E of x) E.catch(() => {});
		const D = [];
		for (let E = 0; E < v.length; E += 1)
			if (v[E])
				try {
					...
					if (I instanceof Ve) return { type: 'redirect', location: I.location };
					let A = 500,
						j;
					if (h != null && h.includes(I)) (A = I.status ?? A), (j = I.error);
					else if (I instanceof ee) (A = I.status), (j = I.body);
					else {
						if (await V.updated.check()) return await B(r);
						j = await X(I, { params: a, url: r, route: { id: s.id } });
					}
					const G = await Ue(E, D, c);
					return G
						? await Y({
								url: r,
								params: a,
								branch: D.slice(0, G.idx).concat(G.node),
								status: A,
								error: j,
								route: s
						  })
						: await Ne(r, { id: s.id }, j, A);
				}
			else D.push(void 0);
		return await Y({
			url: r,
	...
	async function ie({ status: e, error: n, url: r, route: a }) {
		...
		if (t.server_loads[0] === 0)
			try {
				...
				(r.origin !== location.origin || r.pathname !== location.pathname || C) && (await B(r));
			}
		const m = await de({
				loader: i,
				url: r,
			...
			v = { node: await d(), loader: d, universal: null, server: null, data: null };
		return await Y({ url: r, params: s, branch: [m, v], status: e, error: n, route: null });
	}
	function W(e, n) {
		if (_e(e, J)) return;
		const r = se(e);
		for (const a of u) {
			const s = a.exec(r);
			if (s) return { id: e.pathname + e.search, invalidating: n, route: a, params: Qe(s), url: e };
		}
	}
	function se(e) {
		return Ze(e.pathname.slice(J.length) || '/');
	}
	function Te({ url: e, type: n, intent: r, delta: a }) {
		var m, v;
		let s = !1;
		const c = {
			from: {
			...
			to: {
				...
				route: { id: ((v = r == null ? void 0 : r.route) == null ? void 0 : v.id) ?? null },
				url: e
			},
			willUnload: !r,
			type: n
	...
	async function ce({
		...
		var x, D, E;
		const R = W(e, !1),
			L = Te({ url: e, type: c, delta: g, intent: R });
		if (!L) {
			y();
			return;
		}
		const O = $;
		v(), (P = !0), T && V.navigating.set(L), (ne = m);
		let _ = R && (await he(R));
		if (!_) {
		...
		if (_.type === 'redirect')
			...
			((x = _.props.page) == null ? void 0 : x.status) >= 400 &&
				(await V.updated.check()) &&
				(await B(e));
		if (
			((S.length = 0),
			(H = !1),
			...
			if (
				...
				let A = $ + 1;
				for (; Z[A] || K[A]; ) delete Z[A], delete K[A], (A += 1);
			}
		}
		(b = null),
			T ? ((p = _.state), _.props.page && (_.props.page.url = e), F.$set(_.props)) : Oe(_);
		...
		!r && !U && Ee(),
			(k = !0),
			_.props.page && (M = _.props.page),
			(P = !1),
			c === 'popstate' && Ae($),
			w.after_navigate.forEach((I) => I(L)),
			V.navigating.set(null),
			(N = !1);
	}
	async function Ne(e, n, r, a) {
		return e.origin === location.origin && e.pathname === location.pathname && !C
	...
	function Ye() {
		...
		f.addEventListener('mousedown', n), f.addEventListener('touchstart', n, { passive: !0 });
		const r = new IntersectionObserver(
			(c) => {
				for (const g of c)
					g.isIntersecting && (oe(se(new URL(g.target.href))), r.unobserve(g.target));
			},
			{ threshold: 0 }
		);
		function a(c, g) {
		...
		function s() {
			...
			for (const c of f.querySelectorAll('a')) {
				...
				const y = le(c);
				y.reload ||
					(y.preload_code === Fe.viewport && r.observe(c),
					y.preload_code === Fe.eager && oe(se(g)));
			}
		}
			...
			(history.scrollRestoration = 'manual'),
				...
				f.addEventListener('click', (n) => {
					...
					if (
						...
						return;
					if (s || m.reload) {
						Te({ url: a, type: 'link' }) ? (P = !0) : n.preventDefault();
						return;
					}
					...
					ce({
						...
						redirect_chain: [],
						details: { state: {}, replaceState: m.replace_state ?? a.href === location.href },
				...
				f.addEventListener('submit', (n) => {
					...
					O && L.append(O, (a == null ? void 0 : a.getAttribute('value')) ?? ''),
						...
						ce({
							...
							keepfocus: m ?? !1,
							redirect_chain: [],
							details: { state: {}, replaceState: R ?? c.href === location.href },
							nav_token: {},
							accepted: () => {},
							blocked: () => {},
							type: 'form'
				...
				addEventListener('popstate', async (n) => {
					...
					if ((r = n.state) != null && r[q]) {
						...
						if (p.url.href.split('#')[0] === location.href.split('#')[0]) {
							(K[$] = Q()), ($ = n.state[q]), scrollTo(a.x, a.y);
							return;
						}
						const s = n.state[q] - $;
						await ce({
				...
				addEventListener('hashchange', () => {
					z && ((z = !1), history.replaceState({ ...history.state, [q]: ++$ }, '', location.href));
				});
		...
		_hydrate: async ({
			...
			try {
				const y = r.map(async (O, _) => {
						const h = c[_];
						return (
							h != null && h.uses && (h.uses = Be(h.uses)),
							de({
								loader: t.nodes[O],
								url: m,
								params: a,
								...
								parent: async () => {
									const U = {};
									for (let x = 0; x < _; x += 1) Object.assign(U, (await y[x]).data);
									return U;
								},
				...
				if (y instanceof Ve) {
					await B(new URL(y.location, location.href));
					return;
				}
				v = await ie({
	...
	return new Promise(async (d) => {
		...
		function b(C) {
			return vt(C, {
				Promise: (T) =>
					new Promise((k, N) => {
						f.set(T, { fulfil: k, reject: N });
					})
			});
		}
		let w = '';
		for (;;) {
			const { done: C, value: T } = await S.read();
			if (C && !w) break;
			for (
				w +=
					!T && w
					...
					(p = N.nodes) == null ||
						p.forEach((P) => {
							(P == null ? void 0 : P.type) === 'data' &&
								((P.uses = Be(P.uses)), (P.data = b(P.data)));
						}),
						d(N);
				else if (N.type === 'chunk') {
					const { id: P, data: z, error: H } = N,
						F = f.get(P);
					f.delete(P), H ? F.reject(b(H)) : F.fulfil(b(z));
				}
			}
		}
	});
